    ...
    case transactionState of
      Transactional ->
        liftIO $ withResource baseContext.dbPool $ \dbConn ->
          let transactionContext = appContext {AppContext.dbConnection = Just dbConn}
           in guardAppContext transactionContext (runExceptT $ runLogging loggingLevel $ runReaderT (AppContext.runAppContextM function) transactionContext)
      NoTransaction -> liftIO $ guardAppContext appContext (runExceptT $ runLogging loggingLevel $ runReaderT (AppContext.runAppContextM function) appContext)
  case eResult of
