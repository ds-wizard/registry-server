...
runPersistentCommands
  ...
  -> [String]
  -> m ()
runPersistentCommands runAppContextWithAppContext' updateContext execute components = do
  checkPermission _DEV_USE_ROLE_PERMISSION
  commands <- findPersistentCommandsForRetryByStates components
  unless
    (null commands)
    ( do
        traverse_ (runPersistentCommand runAppContextWithAppContext' updateContext execute False) commands
        runPersistentCommands runAppContextWithAppContext' updateContext execute components
    )
...
runPersistentCommand
  ...
  -> m ()
runPersistentCommand runAppContextWithAppContext' updateContext execute force commandSimple = do
  context <- ask
...
runPersistentCommandChannelListener
  ...
  -> [String]
  -> m ()
runPersistentCommandChannelListener runAppContextWithAppContext updateContext execute components = do
